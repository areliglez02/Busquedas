    ...
    do{
        ...
        case 2:
            if (!solicitudCola.vacia()) {
                solicitud = solicitudCola.dequeue();  // Quitar la solicitud de la cola.
        ...
        case 3:
            if (!solicitudCola.vacia()) {
                std::cout << "Buscar solicitud: " << std::endl;
                std::cin >> datos;
                posicion = solicitudCola.busquedaLineal(datos);

                if (posicion != -1) {
                    std::cout << "Solicitud encontrada en la posición " << posicion << ": " << std::endl;
                }
                else {
                    std::cout << "Solicitud no encontrada. Favor de registrar su solicitud." << std::endl;
                }
            }
            else {
                std::cout << "La cola de solicitudes está vacía." << std::endl;
